...
class TreeNode:
    ...
    def __repr__(self):
        return (
            f := lambda x: (
                x and f"TreeNode{{val: {x.val}, left: {f(x.left)}, right: {f(x.right)}}}" or "None"
            )
        )(self)
    ...
    def parse(self):
        ...
        nodes = [None if x is None else TreeNode(x) for x in self]
        kids = nodes[::-1]
...
class ListNode:
    ...
    def parse(self):
        if not self:
            return None
        if type(self) == ListNode:
            return self
        self = [*self]
        sub_nodes = ListNode.parse(self[1:])
        list_node = ListNode(self[0], sub_nodes)
        return list_node
...
def test(
    ...
    def vc(func, name, v):
        ...
        def is_gen(v):
            return hasattr(v, "__iter__") and not hasattr(v, "__len__")

        def is_iter(v):
            return type(v) in (tuple, set, list, dict, deque) or is_gen(v)

        def to_list(v):
            return v if not is_iter(v) else [to_list(x) for x in v]
    ...
    def vcast(func, args, init=None):
        ...
        if init:
            ...
            args = [
                vc(func, func.__code__.co_varnames[i + d], x) for i, x in enumerate(args[:argc])
            ]
    ...
    def print_res(passed, res, expected, *args):
        def g(t):
            return str(t.serialize(t) if type(t) is ListNode or type(t) is TreeNode else t)

        def c(c, t, w=50):
            return "\x1b[{1}m{2}\x1b[0m".format(
                s := g(t), 30 + c, s[:w] + "..." if len(s) >= w else s
            )

        e = 2 if passed else 1
        print(
            "{} args {} result {} expected {}".format(
    ...
    custom_class_tests = classname is not None and "Launcher" not in str(classname) and custom
    ...
    if not custom_class_tests:
        for s in text.splitlines():
            if s.startswith("Input:"):
                ...
                s == "" or any(s.startswith(t) for t in ("Exampl", "Explan", "Operat", "Note", "#"))
            ):
